package com.yp.security.code.img;

import cn.hutool.core.util.StrUtil;
import com.yp.common.constant.PreConstant;
import com.yp.common.exception.ValidateCodeException;
...
public class ImageCodeFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        // 必须为/login请求和post请求
        if (StrUtil.equals("manage/login", request.getRequestURI()) && StrUtil.isEmpty(token) && StrUtil.equalsIgnoreCase(request.getMethod(), "POST")) {
            try {
                // 1. 进行验证码的校验
